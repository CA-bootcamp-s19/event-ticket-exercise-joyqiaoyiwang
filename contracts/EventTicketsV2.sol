...
contract EventTicketsV2 {
    ...
    */
    function addEvent(string memory description, string memory URL, uint numTickets) public verifyOwner() {
        events[idGenerator] = Event(description, URL, numTickets, 0, true);
        idGenerator++;
    ...
    */
    function buyTickets(uint eventId, uint numTickets) public payable {
        ...
        require(thisEvent.isOpen);
        require(numTickets <= thisEvent.totalTickets - thisEvent.sales);
        require(numTickets * PRICE_TICKET <= msg.value);
        thisEvent.sales += numTickets;
        thisEvent.buyers[msg.sender] += numTickets;
        if (msg.value > numTickets * PRICE_TICKET) {
    ...
    */
    function getRefund(uint eventId) public {
        Event storage thisEvent = events[eventId];
        require(thisEvent.buyers[msg.sender] > 0);
        uint numTickets = thisEvent.buyers[msg.sender];
        thisEvent.totalTickets += numTickets;
        msg.sender.transfer(thisEvent.buyers[msg.sender] * PRICE_TICKET);
    ...
    */
    ...
    returns(uint ticketCount) {
        ticketCount = events[eventId].buyers[msg.sender];
    }
    /*
        ...
        TODO:
            ...
            - emit the appropriate event
    */
    function endSale(uint eventId) public payable verifyOwner() {
        Event storage thisEvent = events[eventId];
        uint balanceTransferred = msg.value + thisEvent.sales * PRICE_TICKET;
        thisEvent.isOpen = false;
        msg.sender.transfer(balanceTransferred);
        emit LogEndSale(msg.sender, balanceTransferred, eventId);
    }
}
