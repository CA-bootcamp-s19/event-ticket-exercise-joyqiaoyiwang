...
contract EventTickets {
    ...
    */
    ...
    event LogForSale(address owner, uint balanceTransferred);
    ...
    */
    constructor(string memory description, string memory URL, uint numTickets) public {
        owner = msg.sender;
        myEvent = Event(description, URL,  numTickets, 0, true);
  }
    /*
    ...
    */
    ...
    returns(uint ticketCount) {
        ticketCount = myEvent.buyers[buyer];
    }
    /*
    ...
    */
    function buyTickets(uint numTickets) public payable {
        ...
        require(numTickets <= myEvent.totalTickets - myEvent.sales);
        require(numTickets * TICKET_PRICE <= msg.value);

        myEvent.sales += numTickets;
        myEvent.buyers[msg.sender] += numTickets;
        if (msg.value > numTickets * TICKET_PRICE) {
    ...
    */
    ...
    function endSale() public payable verifyOwner() {
        uint balanceTransferred = msg.value + myEvent.sales * TICKET_PRICE;
        myEvent.isOpen = false;
        msg.sender.transfer(balanceTransferred);
        emit LogForSale(msg.sender, balanceTransferred);
    }
}
